...
### centrifuger classification command examples
  ...
  ./centrifuger -x cfr_idx -u "path_to_10x_fastqs/*_R2_*.fastq.gz" --barcode "path_to_10x_fastqs/*_R1_*.fastq.gz" --UMI "path_10x_fastqs/*_R1_*.fastq.gz" --read-format bc:0:15,um:16:-1 --barcode-whitelist cellranger_folder/cellranger-cs/VERSION/lib/python/cellranger/barcodes/3M-february-2018.txt.gz > output.tsv # 10x Genomics single-end fastq file
  ...
  samtools view -f 0xC alignment.bam | samtools sort -n - | samtools fastq - | ./centrifuger -x cfr_idx --read-format bc:hd:CB:5:-1 --un unclassified -i - > output.tsv # unmapped read fragment from paired-end alignment BAM file generated by 10x Genomics Cellranger. Also output unclassified reads to files with prefix unclassified.
